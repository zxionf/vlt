            ...
            Button(
                onClick = {
                    if (password.length < 6) {
                        errorText = "主密码至少需要 6 位"; return@Button
                    }
                    if (password != confirmPassword) {
                        errorText = "两次输入的密码不一致"; return@Button
                    }
                    isProcessing = true
                    scope.launch {
                        try {
                            withContext(Dispatchers.IO) {
                                ...
                                val publicKeyStr =
                                    CryptoManager.publicKeyToString(rsaKeyPair.public)
                                val privateKeyStr =
                                    CryptoManager.privateKeyToString(rsaKeyPair.private)

                                val encPriv = CryptoManager.encrypt(privateKeyStr, kMaster)
                                val magicEnc =
                                    CryptoManager.encrypt(CryptoManager.MAGIC_TEXT, kMaster)
                                val dataKey = CryptoManager.generateDataKey()
                                val encDk =
                                    CryptoManager.rsaEncrypt(dataKey.encoded, rsaKeyPair.public)
                                val encryptedDataKeyStr =
                                    "${CryptoManager.bytesToBase64(encDk.iv)}:${
                                        CryptoManager.bytesToBase64(encDk.ciphertext)
                                    }"
                                val deviceId = UUID.randomUUID().toString()

                                val db = PwdDB.getInstance(context)
                                db.KeyPairDao().insert(
                                    KeyPairEntity(
                                        ...
                                        encryptedPrivateKey = "${CryptoManager.bytesToBase64(encPriv.iv)}:${
                                            CryptoManager.bytesToBase64(
                                                encPriv.ciphertext
                                            )
                                        }",
                                ...
                                db.DeviceDao().insert(
                                    DeviceEntity(
                                        ...
                                        encryptedDataKey = encryptedDataKeyStr,
                            ...
                            errorText =
                                "初始化失败: ${e.javaClass.simpleName}: ${e.message ?: "未知错误"}"
                        } finally {
                            isProcessing = false
                        }
                    }
